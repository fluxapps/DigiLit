...
class xdglLibrarianGUI
{
    ...
    public function executeCommand()
    {
        ...
        switch ($cmd) {
            ...
            case self::CMD_UPDATEASSIGNMENT:
                ...
                $this->tabs_gui->setBackTarget($this->pl->txt('librarian_back'),
    ...
    protected function updateAssignments()
    {
        ...
        foreach (xdglLibrarian::where(array('library_id' => $lib_id))->where(array('usr_id' => $usr_ids),
            'NOT IN')->get() as $obj) {
            if ($obj->isDeletable()) {
                $obj->delete();
            }
    ...
    protected function buildFormList()
    {
        ...
        $role_ids = array_merge(xdglConfig::getConfigValue(xdglConfig::F_ROLES_MANAGER),
            xdglConfig::getConfigValue(xdglConfig::F_ROLES_ADMIN));
        ...
        $q = "SELECT ua.usr_id, usr.firstname, usr.lastname, usr.email, lib.library_id AS assigned_to
				...
				LEFT JOIN " . xdglLibrarian::TABLE_NAME . " lib ON lib.usr_id = ua.usr_id AND lib.library_id = " . $ilDB->quote($lib_id,
                'integer') . "
				WHERE  " . $ilDB->in('ua.rol_id', array_values($role_ids), false, 'integer') . " GROUP BY ua.usr_id";

        $a_set = $ilDB->query($q);
        while ($rec = $ilDB->fetchObject($a_set)) {
            $cb = new ilCheckboxInputGUI($rec->lastname . ', ' . $rec->firstname . ' (' . $rec->email . ')',
                'usr_id[]');
